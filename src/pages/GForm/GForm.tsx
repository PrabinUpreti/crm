...
const GForm: React.FC = () => {
  ...
  const [additionalTitleAdded, setAdditionalTitleAdded] = useState(false);
  ...
  const addField = (type: string) => {
    const initialOptions =
      type === "multiple_choice" || type === "checkbox"
        ? ["Option 1", "Option 2"]
        : [];
    const newField = { id: nextId, label: "", type, options: initialOptions };
  ...
  const addAdditionalTitle = () => {
    if (!additionalTitleAdded) {
      setAdditionalTitleAdded(true);
    }
  };
      ...
      {fields.length > 0 && (
        <div className="flex gap-2 justify-end mt-4">
          ...
          <Button
            ...
            variant={"default"}
            size={"lg"}
            className="px-10 py-2 bg-green-500 text-white rounded-md hover:bg-blue-600"
          >
            Publish
          </Button>
        </div>
